	...
	},
	{
		...
		use: function () {
			if (!items[3].timeout) {
				...
				setTimeout(function () {
					if (
						particles[Math.floor(items[3].currentBullet.y)][
							Math.floor(items[3].currentBullet.x)
						] >= 1
					) {
						particles[Math.floor(items[3].currentBullet.y)][
							Math.floor(items[3].currentBullet.x)
					...
					setTimeout(function () {
						for (var h = 0; h < items[3].tinyBullets.length; h++) {
							if (
								particles[Math.floor(items[3].tinyBullets[h].y)][
									Math.floor(items[3].tinyBullets[h].x)
								] >= 1
							) {
								particles[Math.floor(items[3].tinyBullets[h].y)][
									Math.floor(items[3].tinyBullets[h].x)
							...
							for (var j = entities.length - 1; j >= 3; j--) {
								...
								if (
									entities[j].x <
										items[3].tinyBullets[h].x + items[3].tinyBullets[h].width &&
									entities[j].x + entities[j].width >
										items[3].tinyBullets[h].x &&
									entities[j].y <
										items[3].tinyBullets[h].y +
											items[3].tinyBullets[h].height &&
									entities[j].height + entities[j].y >
										items[3].tinyBullets[h].y &&
	...
	},
	{
		...
		use: function () {
			...
			if (r == 1) {
				...
				for (
					var i = Math.floor(Math.max(entities[0].y - 50, 5));
					i < Math.min(465, entities[0].y + 50);
					i++
				) {
					for (
						var j = Math.floor(Math.max(entities[0].x - 50, 5));
						j < Math.min(395, entities[0].x + 50);
						j++
					) {
				...
				for (var i = 0; i < 100; i++) {
					...
					for (var h = 0; h < player.height - 1; h++) {
						for (var g = 0; g < player.width; g++) {
							if (
								Math.abs(
									particles[Math.floor(entities[0].y) + h][
										Math.floor(entities[0].x) + g + i
									]
								) > 2
							) {
								temp = false;
